...
public abstract class NettyRemotingAbstract {
	 ...
	 * which protects system memory footprint.
	 */
	 ...
	 * incoming request, we may look up the responding processor in this map to
	 * handle the request.
	 */
	 ...
	 * @throws Exception
	 *             if there were any error while processing the incoming
	 *             command.
	 */
		...
		if (pair != null) {
			...
			try {
				...
				if ((System.currentTimeMillis() % 10000) == 0) {
					log.warn(RemotingHelper.parseChannelRemoteAddr(ctx.channel()) + ", too many requests and system thread pool busy, RejectedExecutionException " + pair.getObject2().toString()
	 ...
	 *
	 * @return Dedicated thread pool instance if specified; or null if the
	 *         callback is supposed to be executed in the netty event-loop
	 *         thread.
	 */
	...
	/**
	 * <p>
	 * This method is periodically invoked to scan and expire deprecated
	 * request.
	 * </p>
	 */
	 ...
	 * 
	 * @param channel
	 *            the channel which is close already
	 */
	...
	public void invokeOnewayImpl(final Channel channel, final RemotingCommand request, final long timeoutMillis) throws InterruptedException, RemotingTooMuchRequestException,
		...
		if (acquired) {
			...
			if (timeoutMillis <= 0) {
				...
				String info = String.format("invokeOnewayImpl tryAcquire semaphore timeout, %dms, waiting thread nums: %d semaphoreAsyncValue: %d", timeoutMillis,
						this.semaphoreOneway.getQueueLength(), this.semaphoreOneway.availablePermits());
				log.warn(info);
				throw new RemotingTimeoutException(info);
			}
		}
